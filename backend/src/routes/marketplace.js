...
// ─── FILE UPLOAD CONFIG ─────────────────────────────────────────────────────
//
//  PRIMARY (production-safe):
//    Cloudinary CDN. Activated by setting CLOUDINARY_URL in .env.
//    Returns full https://res.cloudinary.com/... URLs that survive Railway
//    redeploys and serve via global CDN.
//
//  FALLBACK (dev only, when CLOUDINARY_URL is unset):
...
//
//  See backend/src/services/cloudinary.js for upload/delete helpers.
// ─────────────────────────────────────────────────────────────────────────────

const cloudinaryService = require('../services/cloudinary');

// Ensure uploads directory exists (used only when Cloudinary is disabled)
const fs = require('fs');
...
// Use Cloudinary storage if configured, else fall back to disk
const storage = cloudinaryService.enabled
  ? cloudinaryService.makeStorage()
  : diskStorage;

if (cloudinaryService.enabled) {
  console.log(`[upload] Cloudinary active (folder: ${cloudinaryService.FOLDER}/listings)`);
} else {
  console.warn('[upload] CLOUDINARY_URL not set — falling back to disk storage (dev only)');
}
...
router.post('/upload', authMiddleware, upload.array('files', 5), (req, res) => {
  try {
    ...
    // Cloudinary path: multer-storage-cloudinary populates `path` with the full
    //   secure HTTPS URL (e.g. https://res.cloudinary.com/.../listing.jpg).
    // Disk path: file.filename is the local filename, served via /uploads/ proxy.
    const urls = req.files.map(f => {
      if (cloudinaryService.enabled) return f.path;            // full Cloudinary URL
      return `/uploads/${f.filename}`;                          // dev fallback
    });
    res.json({ urls });
  } catch (err) {
